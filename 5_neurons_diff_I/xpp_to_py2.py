...
fig, ax = plt.subplots(3, 1, figsize=(14,8), sharey='row', sharex='col')
...
times = []
V1, V2, V3, V4, V5 = [], [], [], [], []
with open('diff_I_wo_coupling.dat', newline='') as file:
    datareader = csv.reader(file, delimiter=' ')
    for row in datareader:
        if float(row[0]) <= 45 :
            times.append(float(row[0]))
...
ax[2].set_xlabel('Time ($10^{-2}$ seconds)', size=10)
ax[1].set_ylabel('Voltage $V_k, k \in \{1,..,5\}$', size=10)
ax[2].set_ylabel('Voltage $V_k, k \in \{1,..,5\}$', size=10)

ax[1].text(0., 1.35, 'With Coupling')
ax[2].text(0., 1.35, 'Without Coupling')

ax[0].set_ylabel('Current $I_k, k \in \{1,..,5\}$', size=10)
ax[0].set_ylim(.88)

fig.suptitle('Network of 5 electrically coupled neurons, $\\beta=0.1$ and $\gamma=0.1$', size=14)
ax[0].legend(loc='upper right')
ax[1].legend(loc='upper right')
ax[2].legend(loc='upper right')

plt.savefig('5_neurons_different_I_.svg')
plt.show()
