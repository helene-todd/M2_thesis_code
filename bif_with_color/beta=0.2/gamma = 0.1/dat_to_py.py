...
if len(args.mesh) > 0 :
    ...
    plt.pcolormesh(data['I'], data['phi'], data['cycles'], cmap='viridis', shading='smooth', edgecolors=None, norm = colors.SymLogNorm(linthresh=0.03, linscale=0.03, vmin=min_val, vmax=10**3, base=10))
    plt.colorbar()
    print(max_val)
...
plt.title('Bifurcation diagram for two coupled neurons, $\gamma=0.1, \\beta=0.2$', fontsize=11)
