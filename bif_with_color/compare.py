...
fig, ax = plt.subplots(2, 2, figsize=(12,8), sharey='row', sharex='col')
...
ax[0,1].set_title('$\gamma$=0.4, $\\beta$=0.1', fontsize=13)
...
ax[1,1].set_title('$\gamma$=0.4, $\\beta$=0.2', fontsize=13)
ax[1,1].set_xlabel('Current $I$', size=12)

im = ax[1,1].pcolormesh(data['I'], data['phi'], data['cycles'], cmap='viridis', shading='smooth', edgecolors=None, norm = colors.SymLogNorm(linthresh=0.03, linscale=0.03, vmin=min_val, vmax=max_val, base=10))
...
plt.suptitle('Bifurcation diagrams with convergence speed', size=16)
fig.tight_layout()
...
plt.savefig('comparing_bifs_cv_speed.png', dpi=600)
plt.show()
