...
k = 0
with open('keeping_phase.dat', newline='') as file:
    datareader = csv.reader(file, delimiter=' ')
    for row in datareader:
        if float(row[0]) >= 0 and float(row[0]) <= 20 and k%20 == 0 :
        ...
        if float(row[0]) >= 50 and float(row[0]) <= 70 and k%20 == 0 :
...
ax[0, 1].plot(times_plot2, I1_plot2, alpha=1, color=c[0], linestyle='-', label='$I_1$') #alpha=0.75
ax[0, 1].plot(times_plot2, I2_plot2, alpha=1, color=c[1], linestyle='-', label='$I_2$') #alpha=0.75
...
fig.suptitle('From A/S regime to S regime', size=14)
