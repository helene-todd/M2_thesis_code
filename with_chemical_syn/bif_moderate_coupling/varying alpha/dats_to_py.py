...
# USAGE : Run in terminal "python dats_to_py.py alpha=2.dat alpha=2.5.dat alpha=3.dat alpha=3.5.dat alpha=4.dat line.dat stable1.dat stable2.dat".
...
for k in range(len(I)) :
    if phi[k][1] != 0.5 and phi[k][1] != 1 and phi[k][1] != 0 :
        plt.plot(I[k], phi[k], color=c[int(p/2)], linestyle=s[1], label=f'$\\alpha = {alpha[int(p/2)]}$')
        p += 1
...
plt.title('Bifurcation diagram for two coupled neurons, various $\\alpha$', fontsize=15, pad=15)
