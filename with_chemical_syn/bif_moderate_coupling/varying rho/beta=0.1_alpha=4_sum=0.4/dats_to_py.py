...
# USAGE : Run in terminal "python dats_to_py.py gamma=0_kappa=0.4.dat gamma=0.1_kappa=0.3.dat gamma=0.2_kappa=0.2.dat gamma=0.3_kappa=0.1.dat  gamma=0.4_kappa=0.dat line.dat stable1.dat stable2.dat".
...
for k in range(len(I)) :
    if phi[k][1] != 0.5 and phi[k][1] != 1 and phi[k][1] != 0 :
        plt.plot(I[k], phi[k], color=c[int(p/2)], linestyle=s[1], label=f'$r = {rho[int(p/2)]}$')
        p += 1

    else :
        if stability[k] == 1 :
            plt.plot(I[k], phi[k], color=c[int(p/2)], linestyle=s[0]) #linestyle=s[stability[k]-1]
        if stability[k] == 2 :
            plt.plot(I[k], phi[k], color=c[int(p/2)], linestyle=s[1])

plt.title('Bifurcation diagram for two coupled neurons, $\gamma=0.4, \\beta=0.1, \\alpha=2$', fontsize=16, pad=15)
