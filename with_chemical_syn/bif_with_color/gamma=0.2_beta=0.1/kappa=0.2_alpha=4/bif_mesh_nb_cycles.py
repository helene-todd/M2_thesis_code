    ...
    for k in range(len(T)):
        ...
        if continuer == True :
            ...
            if (u1(phi[k], T[k], gamma, beta, kappa, alpha, I) + gamma*beta >= 1 or u2(phi[k], T[k], gamma, beta, kappa, alpha, I) + gamma*beta >= 1) :
                ...
                if abs(res[0]) > 10**(-3) or abs(res[1]) > 10**(-3) :
                    print('--')
                    print('current : ', phi[k], T[k])
                    print('next : ', phi_nxt, T_nxt)
                    [phi_nxt, T_nxt] = fsolve(tosolve, [phi[k]+0.15, T[k]+0.1], args=(phi[k], T[k], gamma, beta, kappa, alpha, I))
                    print('new next : ', phi_nxt, T_nxt)
                    print('--')


                # Try again if previous block didn't solve issue
                res = tosolve([phi_nxt, T_nxt], phi[k], T[k], gamma, beta, kappa, alpha, I)
                if abs(res[0]) > 10**(-3) or abs(res[1]) > 10**(-3) :
                    print('-*-')
                    print('current : ', phi[k], T[k])
                    print('next : ', phi_nxt, T_nxt)
                    [phi_nxt, T_nxt] = fsolve(tosolve, [phi[k]-0.05, T[k]], args=(phi[k], T[k], gamma, beta, kappa, alpha, I))
                    print('new next : ', phi_nxt, T_nxt)
                    print('-*-')
