...
nb_values = 101
currents = np.linspace(1, 2, 101) # currents I, 201
...
for I in currents :
    ...
    for k in range(len(T)):
        ...
        if continuer == True :
            ...
            elif (theo_u1(phi[k], T[k], gamma, beta, kappa, alpha, I) + gamma*beta >= 1 or theo_u2(phi[k], T[k], gamma, beta, kappa, alpha, I) + gamma*beta >= 1 or phi[k] == 0 or phi[k] == 1) :
                if phi[k] <= 0.5 :
                    nb_cycles.append(1)
                elif phi[k] > 0.5 :
                    nb_cycles.append(1)
