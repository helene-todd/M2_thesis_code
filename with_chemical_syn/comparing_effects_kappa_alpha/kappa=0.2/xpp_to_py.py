...
with open('alpha=4.dat', newline='') as file:
    ...
    for row in datareader:
        ...
        if float(row[0]) >= 40 and float(row[0]) <= 60 :
...
ax[0, 1].plot(times_plot2, I1_plot2, alpha=1, color=c[0], linestyle='-', label='$I_{app,1}$') #alpha=0.75
ax[0, 1].plot(times_plot2, I2_plot2, alpha=1, color=c[1], linestyle='-', label='$I_{app,2}$') #alpha=0.75
...
ax[0, 0].set_ylabel('Current $I_{app, k}, k \in \{1,2\}$', size=12)

ax[0, 0].set_ylim(1.,2.)

fig.suptitle('Coupled neurons, $\gamma=0.2, \\beta=0.1, \kappa=0.2, \\alpha=4$', size=14)
